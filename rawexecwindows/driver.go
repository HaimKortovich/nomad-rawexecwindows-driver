...
var (
	...
	errDisabledDriver = fmt.Errorf("raw_exec_windows is disabled")
)

// PluginLoader maps pre-0.9 client driver options to post-0.9 plugin options.
func PluginLoader(opts map[string]string) (map[string]interface{}, error) {
	conf := map[string]interface{}{}
	if v, err := strconv.ParseBool(opts["driver.raw_exec_windows.enable"]); err == nil {
		conf["enabled"] = v
	}
	return conf, nil
}
...
type Config struct {
	// Enabled is set to true to enable the raw_exec_windows driver
...
// NewRawExecWindowsDriver returns a new DriverPlugin implementation
...
func (d *Driver) buildFingerprint() *drivers.Fingerprint {
	...
	if d.config.Enabled {
		...
		attrs["driver.raw_exec_windows"] = pstructs.NewBoolAttribute(true)
	} else {
